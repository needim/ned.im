import { IconBrandBilibili, IconBrandYoutube } from "@tabler/icons-react";
	...
	useEffect(() => {
		if (typeof document !== "undefined") {
			...
			const hasAcknowledged =
				localStorage.getItem("videoSourceAcknowledged") === "true";
			...
			if (!country) {
				console.log(
					"VideoEmbed: No country detected, falling back to language detection",
				);
				const lang = navigator.language || "";
				shouldUseBili = lang.toLowerCase().includes("zh");
				console.log("VideoEmbed: Language detection result:", {
					lang,
					shouldUseBili,
				});
			}
			...
			console.log(
				"VideoEmbed: Final video source:",
				shouldUseBili ? "Bilibili" : "YouTube",
			);
					...
					if (shouldUseBili) {
						...
						toast(
							<div className="flex flex-col gap-2">
								<div className="flex items-center gap-3">
									...
									<p className="font-medium text-zinc-900">
										如果搞错了,请原谅,您仍可在视频窗口内点击切换回 YouTube
									</p>
								</div>
								<button
									onClick={() => {
							...
							</div>,
							{
								duration: 4000,
								className:
									"!bg-gradient-to-r !from-red-400 !to-white/90 !shadow-lg !border-none !rounded-lg animate-in slide-in-from-left-8 duration-300 backdrop-blur-sm",
								position: "top-left",
							},
						);
						...
						toast(
							<div className="flex items-center gap-3">
								...
								<p className="font-medium text-zinc-900">
									看起来您好像来自中国,已为您切换为哔哩哔哩视频源
								</p>
							</div>,
							{
								duration: 4000,
								className:
									"!bg-gradient-to-r !from-[#00A1D6] !to-white/90 !shadow-lg !border-none !rounded-lg animate-in slide-in-from-left-8 duration-300 backdrop-blur-sm",
								position: "top-left",
							},
						);
					} else {
						...
						toast(
							<div className="flex items-center gap-3">
								<IconBrandYoutube className="w-4 h-4 flex-shrink-0 text-zinc-900" />
								<p className="font-medium text-zinc-900">
									看起来您使用的是非中国IP,已为您切换为YouTube视频源
								</p>
							</div>,
							{
								duration: 4000,
			...
			const handleUserInteraction = () => {
				const hasInteracted = document.documentElement.hasAttribute(
