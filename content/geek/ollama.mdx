---
title: "本地部署Deepseekr1无审查版本"
description: "这是我的第一个视频分享，主要介绍了如何使用 Next.js 构建个人博客"
date: "2024-02-05"
videoUrl: "https://www.youtube.com/embed/your-video-id"
attachmentUrl: "/geek/Ollama"
---
下面是在前文基础上经过深入扩展、补充完善后的完整教程。除了详细介绍如何在 Windows、Linux、macOS 等平台上安装和配置 Ollama 以及如何下载与运行 DeepSeek R1 模型外，下文还重点补充了以下两部分内容：

1. **如何清理（卸载）已安装的模型**  
2. **如何对接 WebUI 前端**，以及当前市面上可与 Ollama 模型直接对接的主要 WebUI 项目介绍

下面给出完整教程内容，供读者参考和实践。

---

# 本地部署大模型：基于 Ollama 部署 DeepSeek R1 模型的实战全攻略

本教程旨在提供一份从环境准备、安装配置、模型下载、运行调优到后续清理和前端对接的完整干货。示例中不再使用 Qwen，而是以性能优异的 DeepSeek R1 为示例，同时给出不同模型尺寸对应的硬件建议。重点内容包括如何管理、清理（删除）已安装的模型，以及如何将 Ollama 部署的模型通过 WebUI 工具进行对接和展示。
...
| 型号 | 内存 | CPU | GPU | 适用场景 |
|:-----|:-----|:----|:----|:---------|
| 1.5B 版本 | 8GB+ | i5/Ryzen 5 | 集成显卡或低端独显 | 基础对话、简单文本生成 |
| 7B/8B 版本 | 16GB+ (建议24GB) | i7/Ryzen 7 | RTX 3060及以上 | 通用对话、代码生成 |
| 14B 版本 | 32GB+ | i9/Ryzen 9 | RTX 3080/4070 | 长文本推理、专业应用 |
| 32B 版本 | 64GB+ | 高性能多核服务器 | 双卡专业级GPU | 科研计算、复杂检索 |
| 70B 版本 | 128GB+ | 服务器级多核系统 | 多卡GPU集群 | 企业级私有化部署 |
...
- citeturn1search0 Ollama 清理安装好的模型（CSDN文库）  
- citeturn1search2 少年攻城狮 关于 Ollama 常用命令  
- citeturn1search7 Open WebUI GitHub 项目  
- citeturn1search16 阿里云开发者社区：手把手部署 DeepSeek R1 模型

--- 

这篇文章力求覆盖部署、清理、前端对接等各方面细节，期望能为您的本地大模型应用提供全方位指导。